 ...
 * Story Component.
 *
 * @param {Integer} idx - Current story index.
 * @param {array} stories - All the stories
 * @param {function} setStoryAttributes - Function to update the story in the stories array
 * @param {function} onRemoveStory - Function to remove a story
 * @returns {*} Post HTML.
 */
export const Story = ( { idx, stories, setStoryAttributes, onRemoveStory } ) => {
			...
			<div className={ b( 'overlay-text' ).toString() }>
				<RichText
					onChange={ nextStoryLinkText => {
						setStoryAttributes( idx, { storyLinkText: nextStoryLinkText } );
					} }
